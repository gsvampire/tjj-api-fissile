...
namespace mail;
/**
 * @Description 文件内容描述.-重新部署
 * @author: weiguo.zhou
 * @version: 0.0.1
 * @date: {2016/5/12} {18:11}
 */
class Mail{
    /**
     ...
     * @access public
     * @param string $server 邮件服器域名或IP
     * @param string $username 认证账号
     * @param string $password 认证密码
     * @param int $port 邮件服器端口必须是一个整形数据，默认25
     * @return boolean
     * @throws \NumberFormatException
     * @throws \NumberFormatIntException
     * @throws \ParamEmptyException
     */
    public function setServer($server, $username="", $password="", $port=25) {
        if(empty($server)){
            throw new ParamEmptyException('邮件服务器名称不能空');
        }
        if(empty($username)){
            throw new ParamEmptyException('登录名不能为空');
        }
        if(empty($password)){
            throw new ParamEmptyException('登录密码不能为空');
        }
        if(empty($port)){
            throw new ParamEmptyException('端口不能为空');
        }
        $portInt = self::checkDataIsInt($port);
    ...
    public function setFrom($from) {
        if(empty($from)){
            throw new ParamEmptyException('必须设置发件人地址');
        }
        $this->_from = $from;
        return true;
    }
     ...
     * 设置发件人
     * @access public
     * @param string $fromNameAlias 发件人别名
     * @return boolean
     * @throws \ParamEmptyException
     */
    public function setFromNameAlas($fromNameAlias) {
        if(empty($fromNameAlias)){
            throw new ParamEmptyException('必须设置发件人别名');
        }
        $this->_fromNameAlias = $fromNameAlias;
        return true;
    }
    /**
     * 设置收件人，多个收件人，连续调用多次.
    ...
    public function setReceiver($to) {
        if(empty($to)){
            throw new ParamEmptyException('必须设置收件人地址');
        }
        if(isset($this->_to)) {
    ...
    public function setCc($cc) {
        if(empty($cc)){
            throw new ParamEmptyException('必须设置抄送地址');
        }
        if(isset($this->_cc)) {
     ...
     * @param string $attachment 附件，要发送的本地文件地址 例D:\send.txt
     * @return boolean
     * @throws \ParamEmptyException
     */
    public function setMailInfo($subject, $body, $attachment="") {
        if(empty($subject)){
            throw new ParamEmptyException('必须设置邮件标题');
        }
        if(empty($body)){
            throw new ParamEmptyException('必须设置邮件内容');
        }
    ...
    protected function getCommand() {
        ...
        //设置发件人
        $header = "FROM: ".$this->_fromNameAlias."<" . $this->_from . ">\r\n";
        //设置收件人
     ...
     * @author: weiguo.zhou
     * @version: 0.0.1
     * @date: 2016-01-14 16:00
     * @param string $value 要测试数据
     * @return int 返回转换后的数据
     * @throws \NumberFormatException 参数不为数字
     * @throws \NumberFormatIntException 参数不是一个整数
     * @throws \ParamEmptyException 参数不能为空
     */
    private static function checkDataIsInt($value){
        if(empty($value)){
            throw new ParamEmptyException('要验证的参数不能为空');
        }
        if(!is_numeric($value)){
            throw new NumberFormatException('验证参数必须是一个数字');
        }
        $tempIntValue = intval($value);
        if ($tempIntValue == $value) {
            return $tempIntValue;
        } else {
            throw new NumberFormatIntException('验证参数不是一个整形数据');
        }
    }
